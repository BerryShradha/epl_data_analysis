...
floor(sqrt(ncol(training_data[, -c(19, 22, 23, 24, 25)])))

# train a model with random forest
#   note: number of trees is set to 500
#     and calculation of attributes importance is requested
rf <- randomForest(formula, ntree = 500, importance = T, data = training_data)
rf1 <- randomForest(formula, ntree = 800, mtry = 6, importance = T, data = training_data)
rf2 <- randomForest(formula, ntree = 2000, importance = T, data = training_data)
...
acc_rf
acc_rf1 <- sum(diag(rf_results_table1)) / sum(rf_results_table1)
acc_rf1
acc_rf2 <- sum(diag(rf_results_table1)) / sum(rf_results_table1)
acc_rf2
...
data[idx] <- lapply(data[idx], function(x) as.numeric(x))
...
pred_nn_1 <- compute(nn_1, test_data[, -c(19, 22, 23, 24, 25)])
pred_nn_5 <- compute(nn_5, test_data[, -c(19, 22, 23, 24, 25)])
pred_nn_55 <- compute(nn_55, test_data[, -c(19, 22, 23, 24, 25)])

nn_57 <- neuralnet(formula, hidden = c(5,7), data = training_data)
plot(nn_57)
pred_nn_57 <- compute(nn_57, test_data[,-22])
